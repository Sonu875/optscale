...
CHART_NAME = 'opticloud'
JOB_NAME = 'configurator'
OPTICLOUD_K8S_NAMESPACE = 'default'
...
class Runkube:
    ...
    def versions_info(self):
        if self._versions_info is None:
            self._versions_info = {'opticloud': self.version}
    ...
    def generate_base_overlay(self, update):
        ...
        secret = self.kube_cl.read_namespaced_secret(
            'defaultcert', OPTICLOUD_K8S_NAMESPACE)
        cert = base64.b64decode(secret.data['tls.crt'])
        key = base64.b64decode(secret.data['tls.key'])
        base_overlay = {'opticloud_key': key, 'certificates': {'opticloud': cert}}
    ...
    def remove_old_jobs(self):
        ...
        jobs = batch_api.list_namespaced_job(namespace=OPTICLOUD_K8S_NAMESPACE)
        ...
        for job in matched_jobs:
            ...
            batch_api.delete_namespaced_job(
                name=job, namespace=OPTICLOUD_K8S_NAMESPACE,
            ...
            for job in deleted_jobs.keys():
                ...
                try:
                    batch_api.read_namespaced_job(job,
                                                  namespace=OPTICLOUD_K8S_NAMESPACE)
    ...
    def etcd_pod_name(self):
        ...
        pods = self.kube_cl.list_namespaced_pod(
            OPTICLOUD_K8S_NAMESPACE, label_selector='app=etcd')
    ...
    def delete_configured_key(self):
        ...
        try:
            ...
            resp = k8s_stream(self.kube_cl.connect_get_namespaced_pod_exec,
                              etcd_pod, OPTICLOUD_K8S_NAMESPACE, command=cmd,
    ...
    def check_releases(self, update):
        ...
        opticloud_releases = [r.split()[0] for r in filter(
            lambda x: 'opticloud' in x, out_lines)]
        if len(opticloud_releases) > 1:
            raise Exception(
                "More than 1 opticloud release found: {0}".format(opticloud_releases))
        elif len(opticloud_releases) == 1:
            if opticloud_releases[0] != self.name:
                raise Exception("Specified release name {0} doesn't match "
                                "existing one {1}".format(self.name,
                                                          opticloud_releases[0]))
        elif update:
    ...
    def delete(self):
        ...
        LOG.info("Deleting opticloud cluster %s on k8s %s",
...
if __name__ == '__main__':
    ...
    parser.add_argument('version', help='OptiCloud version')
    ...
    action_group.add_argument('-r', '--restart', action='store_true',
                              help='Remove installed OptiCloud before deploy')
    action_group.add_argument('-d', '--delete', action='store_true',
                              help='Delete installed OptiCloud')
    ...
    parser.add_argument('--with-elk', action='store_true', default=False,
                        help="Start ELK as part of OptiCloud (e.g. in OGI)")
    parser.add_argument('--external-clickhouse', action='store_true',
                        default=False,
                        help='Connect to external clickhouse as part of OptiCloud')
    parser.add_argument('--external-mongo', action='store_true',
                        default=False,
                        help='Connect to external mongodb as part of OptiCloud')
