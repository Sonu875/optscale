...
EXCHANGE_NAME = 'calendar-observers'
QUEUE_NAME = 'calendar-observer'
...
class CalendarObserverWorker(ConsumerMixin):
    ...
    def observe_calendar(self, task):
        ...
        self.rest_cl.observe_calendar(org_id)
        LOG.info('Calendar observe for org %s completed in %s',
                 org_id, time.time() - start_time)

    def process_task(self, body, message):
        try:
            self.observe_calendar(body)
        except Exception as exc:
            LOG.exception('Calendar observer failed: %s', str(exc))
        message.ack()
...
if __name__ == '__main__':
    ...
    with Connection(conn_str) as conn:
        try:
            worker = CalendarObserverWorker(conn, config_cl)
            worker.run()
