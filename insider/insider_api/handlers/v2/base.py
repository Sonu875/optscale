...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def raise405(self):
        raise OptHTTPError(405, Err.OI0003, [self.request.method])
    ...
    def get_arg(self, name, type, default=None, repeated=False):
        try:
            ...
            raise OptHTTPError(400, Err.OI0008, [name])
...
class SecretHandler(BaseHandler):
    ...
    def _check_secret(self, secret, raises=True):
        if raises and not self.secret == secret:
            raise OptHTTPError(403, Err.OI0005, [])
        else:
    ...
    def prepare(self):
        try:
            if not self.secret:
                raise OptHTTPError(401, Err.OI0007, [])
    ...
    @staticmethod
    def raise401():
        raise OptHTTPError(401, Err.OI0006, [])
