...
class TestToken(TestModelBase):
    ...
    def test_token_root(self):
        user_root = User('root@paloaltonetworks.com', self.type_root, 'pass',
                         display_name='I\'m root!', scope_id=None)
        self._check_token(user_root)

    def test_token_partner(self):
        user_partner = User('partner@paloaltonetworks.com', self.type_partner, 'pass',
                            display_name='Partner user',
                            scope_id=str(uuid.uuid4()))
        self._check_token(user_partner)

    def test_token_customer(self):
        user_customer = User('customer@paloaltonetworks.com', self.type_customer, 'pass',
                             display_name='Customer user',
                             scope_id=str(uuid.uuid4()))
        self._check_token(user_customer)

    def test_token_group(self):
        user_group = User('group@paloaltonetworks.com', self.type_group, 'pass',
    ...
    def test_invalid_token(self):
        ...
        user = User('customer@paloaltonetworks.com', self.type_customer, 'pass',
