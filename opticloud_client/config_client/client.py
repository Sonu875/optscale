...
class Client(etcd.Client):
    ...
    def opticloud_email_recipient(self):
        """
        Gets opticloud recipient for service emails
        :return:
        """
        result = self.read_branch("/opticloud_service_emails")
        return result['recipient'] if result['enabled'] == 'True' else None

    def opticloud_error_email_recipient(self):
        """
        Gets opticloud recipient for service emails
        :return:
        """
        result = self.read_branch("/opticloud_error_emails")
        return result['recipient'] if result['enabled'] == 'True' else (
            self.opticloud_email_recipient())
